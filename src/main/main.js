const { app, BrowserWindow, BrowserView, WebContentsView, ipcMain, protocol, session, shell, Menu, MenuItem, dialog, clipboard, net, webContents } = require('electron');
...
let webStore = null;   // Chrome Web Store support REMOVED in 1.5.1 — Pace loads unpacked (folder) extensions only.
...
function isInstallFile(a) { return /\.(pacetheme|paceaddon)$/i.test(String(a || '')); }
function argToUrl(argv) {
  try {
    for (const a of (argv || []).slice(1)) {
      if (!a || a.startsWith('--') || a.startsWith('pace:')) continue;
      if (isInstallFile(a)) continue;   // .pacetheme/.paceaddon are installed, not opened as a tab
...
function argToInstallPath(argv) {
  try {
    for (const a of (argv || []).slice(1)) {
      if (!a || a.startsWith('--')) continue;
      const p = /^file:\/\//i.test(a) ? decodeURIComponent(a.replace(/^file:\/\//i, '')) : a;
      if (isInstallFile(p) && fs.existsSync(p) && fs.statSync(p).isFile()) return path.resolve(p);
    }
  } catch (e) {}
  return null;
}
// Install a .pacetheme (applied immediately) or .paceaddon (after the trust warning), then notify.
// Used by BOTH file associations (double-click) and the addon-shop's install button (auto-install on download).
async function installFromPath(fp) {
  try {
    fp = String(fp || '');
    if (/\.pacetheme$/i.test(fp)) {
      let doc = null; try { doc = JSON.parse(fs.readFileSync(fp, 'utf8')); } catch (e) { return; }
      const r = themes.install(doc);
      if (r && r.ok) { try { themes.apply(r.id); } catch (e) {} try { sendToAll('theme-changed', themes.active()); } catch (e) {} notifyInstall('theme', r.name); }
    } else if (/\.paceaddon$/i.test(fp)) {
      if (!(await addonInstallWarning())) return;
      const r = await addons.installFromZip(fp);
      if (r && r.ok) notifyInstall('addon', (r && r.name) || 'Add-on');
    }
  } catch (e) {}
}
function notifyInstall(kind, name) {
  try {
    const { Notification } = require('electron');
    if (Notification.isSupported()) new Notification({ title: 'Pace', body: (kind === 'theme' ? 'Theme installed: ' : 'Add-on installed: ') + (name || '') }).show();
  } catch (e) {}
  try { if (mainWindow) { if (mainWindow.isMinimized()) mainWindow.restore(); mainWindow.focus(); } } catch (e) {}
}
function maybeInstallFile(fp) {   // route a file path to the installer (defer until the window exists)
  if (!fp) return;
  if (typeof mainWindow !== 'undefined' && mainWindow) installFromPath(fp);
  else pendingInstallPath = fp;
}
...
if (!gotInstanceLock) { app.quit(); }
  ...
  app.on('second-instance', (e, argv) => {
    const ip = argToInstallPath(argv); if (ip) { maybeInstallFile(ip); return; }
    const u = argToUrl(argv); if (u) openInBrowser(u);
    else if (typeof mainWindow !== 'undefined' && mainWindow) { try { if (mainWindow.isMinimized()) mainWindow.restore(); mainWindow.focus(); } catch (_) {} }
  });
  app.on('open-file', (e, p) => { e.preventDefault(); if (isInstallFile(p)) maybeInstallFile(p); else openInBrowser('file://' + p.replace(/\\/g, '/')); }); // macOS
  pendingOpenUrl = argToUrl(process.argv);
  pendingInstallPath = argToInstallPath(process.argv);
}
...
const defaultSettings = {
  ...
  uiLayout: 'top',        // 'top' | 'bottom' — where the address bar / toolbar docks
  tabShape: 'rounded',    // 'rounded' | 'pill' | 'square'
...
// ─── State ───────────────────────────────────────────────────────────────────
let mainWindow   = null;
let tabs         = {};      // tabId -> WebContentsView
...
let lastPageBounds = null;  // last valid page rect from the renderer — applied instantly on tab switch
const VIEW_BG = '#0b0b12';  // dark backdrop for every web view: no white flash while a page attaches/paints
let tabCounter   = 0;
...
// PRIMARY ENGINE: @ghostery/adblocker-electron — the same filter-list engine family real ad
// blockers use, running the full EasyList / EasyPrivacy / uBlock filter sets INCLUDING their
// exception (@@) rules. The exception rules are what stop sites from breaking: our old
// hand-rolled lists had no exception mechanism, which is the structural reason a small
// blocklist still broke the occasional site. Lists are downloaded once, cached on disk, and
// refreshed weekly. If the package isn't installed, everything falls back to the conservative
// built-in blocker below and the browser runs exactly as before.
let AdblockerLib = null, AdbRequest = null;
try { AdblockerLib = require('@ghostery/adblocker-electron'); } catch (e) { AdblockerLib = null; }
try { AdbRequest = require('@ghostery/adblocker').Request; } catch (e) { AdbRequest = (AdblockerLib && AdblockerLib.Request) || null; }
let adEngine = null;   // FiltersEngine instance once lists are loaded
const ADB_TYPE = { mainFrame: 'main_frame', subFrame: 'sub_frame', stylesheet: 'stylesheet', script: 'script',
  image: 'image', font: 'font', object: 'object', xhr: 'xmlhttprequest', ping: 'ping', cspReport: 'csp_report',
  media: 'media', webSocket: 'websocket', other: 'other' };
async function initAdEngine() {
  if (!AdblockerLib || !AdblockerLib.ElectronBlocker) return;
  try {
    const cachePath = path.join(userDataPath, 'adblock-engine.bin');
    try { const st = fs.statSync(cachePath); if (Date.now() - st.mtimeMs > 7 * 24 * 3600e3) fs.unlinkSync(cachePath); } catch (e) {}
    const caching = { path: cachePath, read: fs.promises.readFile, write: fs.promises.writeFile };
    const EB = AdblockerLib.ElectronBlocker;
    // fromPrebuiltFull = ads + tracking + annoyances (cookie walls, anti-adblock). Fall back to
    // ads+tracking if this build of the library doesn't ship the full preset.
    if (typeof EB.fromPrebuiltFull === 'function') adEngine = await EB.fromPrebuiltFull(fetch, caching);
    else adEngine = await EB.fromPrebuiltAdsAndTracking(fetch, caching);
    console.log('[adblock] filter-list engine ready');
  } catch (e) { adEngine = null; console.log('[adblock] engine unavailable, using built-in fallback:', e && e.message); }
}
...
// Registrable domain (eTLD+1) — used to tell first-party from third-party. Handles the common
// two-level public suffixes (co.uk, com.au, …) that the old last-two-labels heuristic got wrong.
const TWO_LEVEL_TLDS = new Set(['co.uk','org.uk','me.uk','net.uk','ac.uk','gov.uk','com.au','net.au','org.au','edu.au','gov.au','co.nz','org.nz','net.nz','co.jp','ne.jp','or.jp','ac.jp','go.jp','com.br','net.br','org.br','gov.br','co.in','net.in','org.in','gov.in','co.kr','or.kr','com.mx','org.mx','com.tr','org.tr','co.za','org.za','com.ar','com.cn','net.cn','org.cn','gov.cn','com.hk','com.tw','com.sg','com.my','co.id','co.th','com.vn','com.ph','com.pk','com.eg','com.sa','co.il','org.il','com.ua','com.pl','com.ru']);
function registrable(h) {
  if (!h) return '';
  const p = h.split('.');
  if (p.length <= 2) return h;
  const lastTwo = p.slice(-2).join('.');
  return TWO_LEVEL_TLDS.has(lastTwo) ? p.slice(-3).join('.') : lastTwo;
}
function applyNetworkRules() {
  ...
  ses.webRequest.onBeforeRequest({ urls: ['<all_urls>'] }, (details, callback) => {
    ...
    if (s.adBlock && details.resourceType !== 'mainFrame') {
      ...
      // Source page URL (for first-party context / engine matching)
      let pageUrl = '', pageHost = '';
      try {
        const w = (details.webContentsId != null) ? webContents.fromId(details.webContentsId) : null;
        if (w && !w.isDestroyed()) { pageUrl = w.getURL() || ''; pageHost = new URL(pageUrl).hostname.toLowerCase(); }
      } catch (e) { pageUrl = ''; pageHost = ''; }

      // ── 0) FILTER-LIST ENGINE (primary path when installed) ──
      // Full EasyList/EasyPrivacy/uBlock rules WITH their exception rules — the engine both blocks
      // far more ads than the fallback lists AND un-breaks sites, because thousands of @@ exception
      // rules exist precisely to protect legitimate resources our crude heuristics couldn't.
      if (adEngine && AdbRequest) {
        try {
          const req = AdbRequest.fromRawDetails({ url, type: ADB_TYPE[rt] || 'other', sourceUrl: pageUrl || undefined });
          const m = adEngine.match(req);
          if (m && m.redirect && m.redirect.dataUrl) return callback({ redirectURL: m.redirect.dataUrl });
          if (m && m.match && !m.exception) return callback({ cancel: true });
          if (m && m.exception) return callback({});   // an @@ exception rule explicitly protects this request
          // No engine opinion → fall through ONLY to explicit user filters + addon rules (below);
          // the crude built-in host/pattern lists are skipped, the engine's lists supersede them.
          if (host) {
            ...
            try {
              ...
              if (rt !== 'media') for (const p of (ar.patterns || [])) { if (lower.indexOf(p) !== -1) return callback({ cancel: true }); }
            } catch (e) {}
          }
          return callback({});
        } catch (e) { /* engine hiccup → fall through to the conservative built-in blocker */ }
      }

      // ── FALLBACK (engine not installed): the conservative built-in blocker ──
      // NEVER block stylesheets, fonts, or media — these are essentially never ads, and blocking one
      // strips a page's CSS/background (text on a blank page) or breaks video.
      if (rt === 'stylesheet' || rt === 'font' || rt === 'media') return callback({});
      // FIRST-PARTY SAFETY: never block a page's own (same-site) requests.
...
function createMainWindow() {
  ...
  mainWindow = new BrowserWindow({
    ...
    webPreferences: {
      ...
      sandbox: false,   // Must stay off — sandbox:true broke startup (Pace wouldn't open).
                        // Trade-off: MV3 extension service workers won't run (documented honestly).
    },
  ...
  mainWindow.webContents.session.on('will-download', (event, item) => {
    ...
    item.once('done', (e, state) => {
      ...
      saveDownloads();
      if (state === 'completed') { if (/\.(pacetheme|paceaddon)$/i.test(savePath)) installFromPath(savePath); }
    });
  });
}
...
function attachTabListeners(view, tabId) {
    ...
    if (/^https?:|^file:/.test(u)) {
      ...
      try {
        if (loadSettings().adBlock && /^https?:/.test(u)) {
          ...
          // Filter-list cosmetic rules: element-hiding CSS + scriptlets from EasyList/uBlock for THIS
          // site — this is what removes the ad placeholders/frames network blocking alone leaves behind.
          if (adEngine && typeof adEngine.getCosmeticsFilters === 'function' && h) {
            try {
              const out = adEngine.getCosmeticsFilters({ url: u, hostname: h, domain: registrable(h),
                getBaseRules: true, getInjectionRules: true, getExtendedRules: false, getRulesFromDOM: false, getRulesFromHostname: true });
              if (out) {
                if (out.styles) wc.insertCSS(out.styles).catch(() => {});
                if (Array.isArray(out.scripts)) for (const code of out.scripts) { try { wc.executeJavaScript(code, true).catch(() => {}); } catch (e) {} }
              }
            } catch (e) {}
          }
        }
  ...
  wc.on('did-start-navigation', (e, url, isInPlace, isMainFrame) => {
    if (isMainFrame) { if (tabMeta[tabId]) tabMeta[tabId].gotFav = false; }
  ...
  wc.on('did-stop-loading', () => {
    ...
    addToHistory(u, t);
    // Favicon fallback: many sites don't fire page-favicon-updated (or do so unreliably). If we still
    // have no icon for this tab, fetch the domain's favicon so the tab shows the real site icon.
    try {
      if (/^https?:\/\//i.test(u) && tabMeta[tabId] && !tabMeta[tabId].gotFav) {
        const host = new URL(u).hostname;
        if (host) {
          if (faviconCache[host]) { try { mainWindow.webContents.send('tab-update', { tabId, favicon: faviconCache[host] }); } catch (e) {} }
          else fetchFaviconData(host).then(data => {
            if (data) { faviconCache[host] = data; persistFavicons();
              if (tabs[tabId] && tabMeta[tabId] && !tabMeta[tabId].gotFav) { try { mainWindow.webContents.send('tab-update', { tabId, favicon: data }); } catch (e) {} } }
          }).catch(() => {});
        }
      }
    } catch (e) {}
  ...
  wc.on('page-favicon-updated', (e, favs) => { if (favs && favs[0]) { if (tabMeta[tabId]) tabMeta[tabId].gotFav = true; try { mainWindow.webContents.send('tab-update', { tabId, favicon: favs[0] }); } catch (e) {} } });
  ...
  wireMedia(wc);
  // Browser shortcuts must work even while a web page (WebContentsView) holds keyboard focus
...
// WebContentsView equivalent of the old setTopBrowserView: re-adding a child view moves it to the
// front of the z-order. Used to keep the sidebar / settings overlays above the active tab view.
...
function createTab(tabUrl = 'pace://newtab', background = false) {
  ...
  tabs[tabId] = view; tabMeta[tabId] = { detached: false, gotFav: false };
  tabOrder.push(tabId);
  attachTabListeners(view, tabId);
  try { if (chromeExtensions) chromeExtensions.addTab(view.webContents, mainWindow); } catch (e) {}
  if (!background) {
    if (activeTabId && tabs[activeTabId]) { try { mainWindow.contentView.removeChildView(tabs[activeTabId]); } catch (e) {} }
    activeTabId = tabId;
    try { mainWindow.contentView.addChildView(view); } catch (e) {}
    try { view.setBounds(lastPageBounds || { x: 0, y: CHROME_H, width: 1200, height: 600 }); } catch (e) {}
...
function switchTab(tabId) {
  ...
  if (activeTabId && tabs[activeTabId] && tabMeta[activeTabId] && !tabMeta[activeTabId].detached) {
    try { mainWindow.contentView.removeChildView(tabs[activeTabId]); } catch (e) {}
  }
  activeTabId = tabId;
  if (tabMeta[tabId]) tabMeta[tabId].detached = false;
  try { mainWindow.contentView.addChildView(tabs[tabId]); } catch (e) {}
  // Give the newly shown view its correct rect IMMEDIATELY (the renderer's pushLayout round-trip
  // arrives a few frames later — that gap is where a switched-to tab could flash or sit stale).
  if (lastPageBounds) { try { tabs[tabId].setBounds(lastPageBounds); } catch (e) {} }
  try { tabs[tabId].webContents.invalidate(); } catch (e) {}
  try { tabs[tabId].webContents.focus(); } catch (e) {}   // keyboard follows the shown tab (no dead-focus limbo after a close)
  const wc = tabs[tabId].webContents;
  // A re-attached local page (pace://newtab) can flash or stick mid entrance-animation. For pace pages,
  // jump the one-shot entrance animations straight to their final state so switching never glitches.
  try { const su = formatUrl(wc.getURL()); if (su && su.startsWith('pace://')) wc.executeJavaScript("try{document.body&&document.body.classList.add('pace-reshown')}catch(_){}", true).catch(()=>{}); } catch (e) {}
...
// Re-entrancy guard: if anything (extension bridge callbacks, spurious duplicate IPC) asks to
// close a tab WHILE a close is already mid-flight, defer it — a nested close corrupts the
// index/neighbor math captured by the outer call.
let closingNow = false;
const pendingCloses = [];
function closeTab(tabId) {
  if (closingNow) { pendingCloses.push(tabId); return; }
  closingNow = true;
  try { closeTabNow(tabId); } finally {
    closingNow = false;
    if (pendingCloses.length) closeTab(pendingCloses.shift());
  }
}
function closeTabNow(tabId) {
  ...
  if (wasActive && tabMeta[tabId] && !tabMeta[tabId].detached) { try { mainWindow.contentView.removeChildView(closingView); } catch (e) {} }
...
function setPageBounds(bounds) {
  ...
  if (!bounds || bounds.width <= 0 || bounds.height <= 0) {
    ...
    if (meta && !meta.detached) { try { mainWindow.contentView.removeChildView(view); } catch (e) {} meta.detached = true; }
    return;
  }
  if (meta && meta.detached) { try { mainWindow.contentView.addChildView(view); } catch (e) {} meta.detached = false; }
  const b = { x: Math.round(bounds.x), y: Math.round(bounds.y), width: Math.max(100, Math.round(bounds.width)), height: Math.max(100, Math.round(bounds.height)) };
  lastPageBounds = b;   // remembered so a newly shown tab gets the correct rect instantly (see switchTab)
  try { view.setBounds(b); } catch (e) {}
  try { view.webContents.invalidate(); } catch (e) {}   // force a repaint so a re-attached page never stays blank
}
...
function setSidebarView(payload) {
  if (!payload || !payload.url) {
    ...
    if (sidebarView && sidebarView._attached) { try { mainWindow.contentView.removeChildView(sidebarView); } catch (e) {} sidebarView._attached = false; }
    return;
  }
  const { url, bounds } = payload;
  if (!sidebarView) {
    sidebarView = new WebContentsView({ webPreferences: { nodeIntegration: false, contextIsolation: true, sandbox: true, backgroundThrottling: false } });
  ...
  if (!sidebarView._attached) { mainWindow.contentView.addChildView(sidebarView); sidebarView._attached = true; }
  // keep sidebar above the active page view
  bringToTop(sidebarView);
...
// Settings shown as a centered popup panel (its own WebContentsView) instead of a full page
function setSettingsPanel(bounds) {
  if (!bounds) {
    if (settingsView) { try { mainWindow.contentView.removeChildView(settingsView); settingsView.webContents.destroy(); } catch (e) {} settingsView = null; }
    return;
  }
  if (!settingsView) {
...
function preloadUrl(rawUrl, force) {
  ...
  const pv = new WebContentsView({ webPreferences: { nodeIntegration: false, contextIsolation: true, sandbox: true, backgroundThrottling: true, preload: path.join(__dirname, '../preload/pagePreload.js') } });
  try { pv.setBackgroundColor(VIEW_BG); } catch (e) {}
...
function navigateTab(tabId, rawUrl) {
  ...
  if (!isInternal && preloadViews[normalized]) {
    ...
    try { mainWindow.contentView.removeChildView(tabs[tid]); } catch (e) {}
    try { tabs[tid].webContents.destroy(); } catch (e) {}
    tabs[tid] = pv;
    if (!tabMeta[tid]) tabMeta[tid] = { detached: false };
    attachTabListeners(pv, tid);
    if (wasActive) {
      try { mainWindow.contentView.addChildView(pv); } catch (e) {}
      if (tabMeta[tid]) tabMeta[tid].detached = false;
    }
...
ipcMain.on('chrome-ready', () => {
  ...
  if (pendingInstallPath) { installFromPath(pendingInstallPath); pendingInstallPath = null; }
...
// Predictive preload: from just 2 typed characters, warm up the site the user most frequently
// visits with that prefix (history keeps one entry per visit, so occurrences ARE the frequency).
ipcMain.on('preload-predict', (e, { prefix } = {}) => {
  try {
    const p = String(prefix || '').toLowerCase().trim();
    if (p.length < 2 || /\s/.test(p)) return;
    if (!loadSettings().fastMode) return;
    const hist = readHistory();
    const counts = {};
    const scan = Math.min(hist.length, 1500);           // recent history only — keeps it instant
    for (let i = 0; i < scan; i++) {
      const it = hist[i]; if (!it || !it.url || !/^https?:\/\//.test(it.url)) continue;
      let host = ''; try { host = new URL(it.url).hostname.replace(/^www\./, ''); } catch (err) { continue; }
      let score = 0;
      if (host.startsWith(p)) score = 2;
      else if (((it.title || '').toLowerCase()).startsWith(p)) score = 1;
      if (!score) continue;
      const c = counts[it.url] || (counts[it.url] = { url: it.url, n: 0 });
      c.n += score;
    }
    const best = Object.values(counts).sort((a, b) => b.n - a.n)[0];
    if (best && best.n >= 3) preloadUrl(best.url, true);  // "frequent" = 2+ host visits (or 3+ title hits)
  } catch (err) {}
});
...
function sendToAll(channel, payload) {
  ...
  // Also deliver to every tab + the settings panel, so internal pages (pace://newtab, themes page)
  // react live — e.g. the new-tab backdrop swaps the moment a theme is applied.
  Object.values(tabs).forEach(v => { try { targets.push(v.webContents); } catch (e) {} });
  try { if (settingsView) targets.push(settingsView.webContents); } catch (e) {}
...
ipcMain.on('media-close', () => {
  if (sidebarView && mediaWC === sidebarView.webContents) {
    try { if (sidebarView._attached) mainWindow.contentView.removeChildView(sidebarView); sidebarView.webContents.destroy(); } catch (e) {}
    sidebarView = null;
...
ipcMain.handle('install-from-store', async (e, { input }) => {
  ...
  if (!webStore || !webStore.installExtension) return { ok: false, reason: 'Installing by ID/link from the Chrome Web Store is no longer supported in Pace. Use “Load Unpacked” to add an extension folder instead.' };
  try {
    let res;
...
ipcMain.handle('open-extension-popup', async (e, { id }) => {
  try {
    ...
    // No popup and no options page → this is an action-only extension (e.g. a game that opens its
    // own tab). Real browsers fire chrome.action.onClicked / chrome.browserAction.onClicked at the
    // background script, which then calls chrome.tabs.create(). Dispatch that click via the
    // electron-chrome-extensions API so those extensions actually do their thing.
    if (!rel) {
      const ok = fireExtensionActionClick(id);
      if (ok) return { ok: true, clicked: true };
      return { ok: false, reason: 'This extension has no popup, and its action click could not be delivered.' };
    }
...
// Deliver a browser-action click to an extension's background context. Tries the
// electron-chrome-extensions API (several method names across versions), so action-only
// extensions (no popup) receive chrome.action.onClicked and can open their own tab.
function fireExtensionActionClick(id) {
  try {
    if (!chromeExtensions) return false;
    const wc = (activeTabId && tabs[activeTabId]) ? tabs[activeTabId].webContents : null;
    // Different library versions expose this differently; try the known shapes.
    const candidates = ['selectExtensionAction', 'activateExtension', 'onBrowserActionClicked', 'clickAction'];
    for (const name of candidates) {
      if (typeof chromeExtensions[name] === 'function') {
        try { chromeExtensions[name](id, wc); return true; } catch (e) {}
      }
    }
    // Newer API: a browserAction sub-object with an activate(extensionId, tabId) method.
    if (chromeExtensions.browserAction && typeof chromeExtensions.browserAction.activate === 'function') {
      try {
        const tabId = wc && wc.id;
        chromeExtensions.browserAction.activate(id, tabId);
        return true;
      } catch (e) {}
    }
    return false;
...
async function helloAvailable() {
  ...
  const script = PS_HELLO_PREAMBLE + "\n"
    ...
    + "  Write-Output ('PACEHELLO=' + $a.ToString())\n"
    + "} catch { Write-Output 'PACEHELLO=ERROR' }";
  const out = await runHelloScript(script);
  const m = String(out || '').match(/PACEHELLO=([A-Za-z]+)/);
  return !!m && m[1] === 'Available';
}
async function helloAuthenticate(reason) {
  ...
  const script = PS_HELLO_PREAMBLE + "\n"
    ...
    + "  Write-Output ('PACEHELLO=' + $r.ToString())\n"
    + "} catch { Write-Output 'PACEHELLO=ERROR' }";
  // Parse a tagged marker rather than the last stdout line: Face ID / camera providers can emit extra
  // trailing output, which made a SUCCESSFUL verification look like 'FAILED'. Any verified method
  // (face, fingerprint, PIN) yields UserConsentVerificationResult.Verified.
  const out = await runHelloScript(script);
  const m = String(out || '').match(/PACEHELLO=([A-Za-z]+)/);
  return !!m && m[1] === 'Verified';
}
...
// Lets the content script know whether the ad blocker is enabled (used by the YouTube ad skipper).
ipcMain.handle('adblock-enabled', () => {
  try { return { enabled: loadSettings().adBlock !== false }; } catch (e) { return { enabled: true }; }
});

// Decide whether to even show the "save password" prompt: only if this login is NEW or its password
// CHANGED. Compares against the stored entry server-side so the page never learns the saved password.
ipcMain.handle('autofill-save-check', (event, { username, password }) => {
  try {
    const s = loadSettings();
    if (s.autofill === false) return { offer: false };
    if (!password) return { offer: false };
    if (!vaultUnlocked || !vaultKey) return { offer: true, locked: true };   // can't compare while locked; de-dupe still applies on save
    const host = frameHost(event);
    if (!host) return { offer: false };
    const v = readVault(); if (!v) return { offer: false };
    const rd = registrableDomain(host);
    const existing = (v.entries || []).find(en => registrableDomain(entryHost(en)) === rd && (en.username || '') === (username || ''));
    if (!existing) return { offer: true, isUpdate: false };
    let stored = ''; try { stored = decryptField(existing.password, vaultKey); } catch (_) {}
    if (stored === password) return { offer: false };   // identical to what's saved → don't nag
    return { offer: true, isUpdate: true };              // password changed → offer to update
  } catch (e) { return { offer: true }; }
});
...
app.whenReady().then(async () => {
  ...
  protocol.handle('pace', (request) => {
    ...
    const file = RENDERER(map[page] || 'newtab.html');
    return net.fetch(require('url').pathToFileURL(file).toString());
  });
  applyNetworkRules();
  initAdEngine().catch(() => {});   // filter-list download/cache happens in the background; blocking works via fallback until ready
  ...
  // contexts AS THEY LOAD. If we load extensions first (Shazam, Helix Jump, …) those APIs are
  // missing in their background contexts, so chrome.tabs.create / chrome.tabCapture do nothing.
  // Creating the instance first fixes that. mainWindow is referenced lazily inside the callbacks,
  // so it's fine that it doesn't exist yet at construction time.
  if (ElectronChromeExtensions) {
    ...
    for (let i = 0; i < attempts.length && !chromeExtensions; i++) {
      try { chromeExtensions = attempts[i](); extLog('ElectronChromeExtensions initialized (before extensions loaded) via attempt #' + (i + 1)); }
      catch (e) { extLog('init attempt #' + (i + 1) + ' failed: ' + String(e && e.message || e)); }
    }
    if (!chromeExtensions) extLog('ElectronChromeExtensions FAILED to initialize — chrome.tabs/tabCapture will not work.');
  } else {
    try { fs.appendFileSync(path.join(userDataPath, 'pace-extensions.log'), '[' + new Date().toISOString() + '] electron-chrome-extensions module NOT installed\n'); } catch (_) {}
  }

  // Chrome Web Store support was removed. Pace loads unpacked extensions from a folder
  // (Load Unpacked) and re-loads previously added folders on startup.
  ...
  // Now that the window exists, register the active tab(s) with the extensions API.
  if (chromeExtensions && mainWindow) {
    try {
    ...
    } catch (e) { try { fs.appendFileSync(path.join(userDataPath, 'pace-extensions.log'), '[' + new Date().toISOString() + '] post-window addTab error: ' + e + '\n'); } catch (_) {} }
  }
