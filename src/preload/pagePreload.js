  ...
  contextBridge.exposeInMainWorld('pace', {
    ...
    on: (channel, fn) => {
      const allowed = ['tab-created', 'tab-closed', 'tab-switched', 'tab-update', 'tab-loading',
        'download-started', 'download-progress', 'download-done', 'window-state', 'settings-changed', 'relayout', 'update-status', 'theme-changed'];
...
// ─── YouTube ad auto-skip (part of the Pace ad blocker) ─────────────────────────
// YouTube video ads are served from YouTube's own CDN, so NO network blocker can remove them
// without breaking the video itself. This is why they "get through": the only reliable fix is
// what uBlock-class blockers do — handle them in the page. This runs at document-start (before
// YouTube's own scripts), then: fast-forwards any playing ad to its end, clicks every variant
// of the Skip button the instant it appears, and mutes the player only while an ad is showing.
(function paceYouTubeAdSkip() {
  try {
    const h = location.hostname || '';
    if (!/(^|\.)youtube\.com$|(^|\.)youtube-nocookie\.com$/.test(h)) return;
    const { ipcRenderer } = require('electron');
    let enabled = true;
    ipcRenderer.invoke('adblock-enabled').then(r => { enabled = !(r && r.enabled === false); }).catch(() => {});
    ipcRenderer.on('settings-changed', (e, s) => { if (s && typeof s.adBlock !== 'undefined') enabled = s.adBlock !== false; });
    let wasMutedByUs = false;
    const SKIP_SEL = '.ytp-skip-ad-button, .ytp-ad-skip-button, .ytp-ad-skip-button-modern, .ytp-ad-skip-button-container button, button[class*="ytp-ad-skip"]';
    setInterval(() => {
      if (!enabled) return;
      try {
        const player = document.querySelector('.html5-video-player');
        const adShowing = !!(player && (player.classList.contains('ad-showing') || player.classList.contains('ad-interrupting')));
        const v = player ? player.querySelector('video') : null;
        if (adShowing && v) {
          if (!v.muted) { v.muted = true; wasMutedByUs = true; }
          try { if (isFinite(v.duration) && v.duration > 0.5) v.currentTime = v.duration; } catch (err) {}
          try { v.playbackRate = 10; } catch (err) {}
          const skip = document.querySelector(SKIP_SEL);
          if (skip) { try { skip.click(); } catch (err) {} }
        } else if (!adShowing && v && wasMutedByUs) {
          try { v.muted = false; } catch (err) {}
          try { if (v.playbackRate > 2) v.playbackRate = 1; } catch (err) {}
          wasMutedByUs = false;
        }
        // Dismiss overlay/text ads on the video itself
        const overlayClose = document.querySelector('.ytp-ad-overlay-close-button');
        if (overlayClose) { try { overlayClose.click(); } catch (err) {} }
      } catch (err) {}
    }, 400);
  } catch (e) {}
})();
